...
class fLDM:
    ...
    def finetune(self, trainers, dataset, test_dataset, bs1, lr1,
        ...
        test_loader = DataLoader(test_dataset, batch_size=bs1, shuffle=False)#batch_size=len(test_dataset)len(test_dataset)
    ...
    def generate(self, fmri_embedding, num_samples, ddim_steps, HW=None, limit=None, state=None):
        ...
        return grid, (255. * torch.stack(all_samples, 0).detach().cpu().numpy()).astype(np.uint8)
