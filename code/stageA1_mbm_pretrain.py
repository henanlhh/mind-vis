...
from config import Config_MBM_fMRI ,Config_MBM_EEG
...
def main(config):
    ...
    dataset = EEGDataset(eeg_signals_path=os.path.join(config.root_path, 'data/eeg/eeg_signals_128_sequential_band_all_with_mean_std.pth'), opt=config)
    # Create loaders
    # 数据加载器。组合了一个数据集和采样器，并提供关于数据的迭代器。
    loaders = {
        split: DataLoader(Splitter(dataset, split_path=os.path.join(config.root_path, 'data/eeg/block_splits_by_image.pth'), split_num=0, split_name=split),
                          batch_size=config.batch_size, drop_last=True, shuffle=True) for split in
        ["train", "val", "test"]}

    # create model
    # config.num_voxels = dataset_pretrain.num_voxels
    model = MAEforFMRI(num_voxels=440, patch_size=config.patch_size, embed_dim=config.embed_dim,in_chans=128,
    ...
    for ep in range(config.num_epoch):
        ...
        cor = train_one_epoch(model, loaders, optimizer, device, ep, loss_scaler, logger, config, start_time, model_without_ddp,
                            img_feature_extractor, preprocess)
        cor_list.append(cor)
        if (ep % 20 == 0 or ep + 1 == config.num_epoch) and ep != 0 and config.local_rank == 0:
            ...
            # plot figures
            # plot_recon_figures(model, device, loaders, output_path, 5, config, logger, model_without_ddp)
