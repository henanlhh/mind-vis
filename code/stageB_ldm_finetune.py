...
def main(config):
    ...
    dataset_train,dataset_test = EEGDataset(eeg_signals_path=os.path.join(config.root_path,
                                                       'data/eeg/eeg_signals_128_sequential_band_all_with_mean_std.pth'),opt=config)
    # # Create loaders
    # # 数据加载器。组合了一个数据集和采样器，并提供关于数据的迭代器。
    # loaders = {
    #     split: DataLoader(
    #         Splitter(dataset, split_path=os.path.join(config.root_path, 'data/eeg/block_splits_by_image.pth'),
    ...
    generative_model = fLDM(pretrain_mbm_metafile, num_voxels=440,
    ...
    #L
    generate_images(generative_model, dataset_train, dataset_test, config)
